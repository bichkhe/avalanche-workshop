...
async fn test_x_chain() -> eyre::Result<()> {
    ...
    // Get blockchain ID
    let blockchain_id =
        make_rpc_call(&client, x_chain_url, "avm.getBlockchainID", json!({})).await?;
    ...
    let utxos = make_rpc_call(
        &client,
        x_chain_url,
        "avm.getUTXOs",
        json!({"addresses": [example_address]}),
    )
    .await?;
    println!(
        "      Example Address UTXOs: {}",
        utxos["utxos"].as_array().map(|arr| arr.len()).unwrap_or(0)
    );
...
async fn test_p_chain() -> eyre::Result<()> {
    ...
    // Get blockchain ID
    let blockchain_id =
        make_rpc_call(&client, p_chain_url, "platform.getBlockchainID", json!({})).await?;
    println!("   ✅ Connected to P-Chain");
    println!("      Blockchain ID: {}", blockchain_id["blockchainID"]);

    // Get network info
    let network_id =
        make_rpc_call(&client, p_chain_url, "platform.getNetworkID", json!({})).await?;
    println!("      Network ID: {}", network_id["networkID"]);

    // Get current supply
    let supply =
        make_rpc_call(&client, p_chain_url, "platform.getCurrentSupply", json!({})).await?;
    let supply_amount: u64 = supply["supply"]
        .as_str()
        .unwrap_or("0")
        .parse()
        .unwrap_or(0);
    println!(
        "      Current Supply: {} AVAX",
        supply_amount as f64 / 1_000_000_000.0
    );

    // Get validators
    let validators = make_rpc_call(
        &client,
        p_chain_url,
    ...
    .await?;
    let validator_count = validators["validators"]
        .as_array()
        .map(|arr| arr.len())
        .unwrap_or(0);
    ...
    let subnet_count = subnets["subnets"]
        .as_array()
        .map(|arr| arr.len())
        .unwrap_or(0);
    ...
    // Get blockchains
    let blockchains =
        make_rpc_call(&client, p_chain_url, "platform.getBlockchains", json!({})).await?;
    let blockchain_count = blockchains["blockchains"]
        .as_array()
        .map(|arr| arr.len())
        .unwrap_or(0);
